<template>
    <div class="flex justify-between items-center mb-4">
        ...
        <Pagination v-if="data != null" :currentPage="data.currentPage" :lastPage="data.lastPage"
            ...
            storage-key="orderStatusCategoriesPerPage"
            @changePage="fetchItems" @perPageChange="handlePerPageChange" />
    </div>
    <BatchButton v-if="selectedIds.length" :selected-ids="selectedIds" :batch-actions="getBatchActions()" />
    <transition name="fade" mode="out-in">
        <div v-if="data != null && !loading" :key="`table-${$i18n.locale}`">
            <DraggableTable table-key="admin.order_status_categories" :columns-config="translatedColumnsConfig" :table-data="data.items"
...
export default {
    ...
    methods: {
        ...
        async fetchItems(page = 1, silent = false) {
            ...
            try {
                const new_data = await OrderStatusCategoryController.getItems(page, this.perPage);
                this.data = new_data;
            } catch (error) {
