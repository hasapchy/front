  ...
  <div>
      ...
      >
        ...
        <template #cards>
          <MapperCardGrid
            ...
            :card-mapper="projectStatusCardMapper"
            title-field="title"
            :title-prefix="projectStatusCardTitlePrefix"
            :selected-ids="selectedIds"
            :show-checkbox="$store.getters.hasPermission('project_statuses_delete')"
    ...
    >
      <ProjectStatusCreatePage
        ref="projectstatuscreatepageForm"
...
const projectStatusesListViewModeMixin = createStoreViewModeMixin({
  listPageKey: 'projectStatuses',
  modes: ['table', 'cards'],
});
...
export default {
  ...
  mixins: [
    ...
    projectStatusesListViewModeMixin,
  ],
  data() {
    return {
  ...
  methods: {
    projectStatusCardTitlePrefix() {
      return '<i class="fas fa-flag text-[#3571A4] mr-1.5 flex-shrink-0"></i>';
    },
    projectStatusCardMapper(item, fieldName) {
      if (!item) return '';
      if (fieldName === 'title') {
        if (item.name) {
          return translateProjectStatus(item.name, this.$t);
        }
