<template>
  <div>
    ...
    <DraggableTable
      table-key="reports.counterparties"
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { name: 'debt_opening', label: 'openingBalance' },
        { name: 'accrued', label: 'accrued' },
  ...
  methods: {
    ...
    async fetchReport() {
      try {
        const data = await ReportsController.getCounterparties({
    ...
    getActiveFiltersCount() {
      return this.getActiveFiltersCountFromConfig([
        ...
        { value: this.mode, defaultValue: 'net' },
        { value: this.currencyMode, defaultValue: 'report' },
      ]);
    },
    itemMapper(item, column) {
      if (['income', 'expense', 'net', 'debt_opening', 'accrued', 'paid', 'debt_closing'].includes(column)) {
    ...
    exportTableToCsv() {
      ...
      link.href = url;
      link.download = `counterparties_report_${new Date().toISOString().slice(0, 10)}.csv`;
      link.click();
