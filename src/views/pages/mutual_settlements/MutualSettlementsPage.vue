                            ...
                            <template #left>
                                <FiltersContainer :has-active-filters="hasActiveFilters"
                                    ...
                                    <div v-if="currencies.length">
                                        ...
                                        <select :value="effectiveCurrencyId"
                                            class="w-full border border-gray-300 rounded px-2 py-1.5 text-sm"
                                            @input="onCurrencyFilterInput">
                                            <option v-for="c in currencies" :key="c.id" :value="c.id">{{ c.symbol }} ({{
                                                c.name }})</option>
                                        </select>
                                    </div>
                                    <div>
                                        <label class="block mb-2 text-xs font-semibold">{{ $t('type') || 'Тип'
                                            }}</label>
...
export default {
    ...
    components: { SideModalDialog, DraggableTable, ClientCreatePage, MutualSettlementsBalanceWrapper, FiltersContainer, CheckboxFilter, TableControlsBar, TableFilterButton, TableSkeleton, draggable: VueDraggableNext },
    data() {
        return {
            allClients: [],
            allClientsRaw: [],
