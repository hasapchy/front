...
export default {
    ...
    data() {
        return {
            ...
            savedSuccessText: this.$t('departmentSaved') || 'Отдел успешно сохранен',
            savedErrorText: this.$t('errorSavingDepartment') || 'Ошибка сохранения отдела',
            deletedSuccessText: this.$t('departmentDeleted') || 'Отдел успешно удален',
            deletedErrorText: this.$t('errorDeletingDepartment') || 'Ошибка удаления отдела',
            departments: [],
            loading: false,
            parentId: null,
            deleteDialog: false,
    ...
    computed: {
        currentCompanyName() {
            return this.$store.getters.currentCompany?.name || 'Моя компания';
        },
        rootDepartments() {
            const map = {};
    ...
    methods: {
        ...
        async confirmDelete() {
            ...
            try {
                ...
                this.showNotification(
                    this.deletedSuccessText || 'Отдел успешно удален',
                    '',
                    false
                );
            } catch (error) {
                ...
                this.showNotification(
                    this.deletedErrorText || 'Ошибка удаления отдела',
                    messages,
                    true
                );
            }
        ...
        async fetchDepartments(page = 1, silent = false) {
            ...
            try {
                ...
                // Обновляем ключ для перерисовки SVG коннекторов
                this.$nextTick(() => {
                    this.connectorsKey++;
                });
            } catch (error) {
                this.showNotification(
                    this.$t('errorLoadingDepartments') || 'Ошибка загрузки отделов',
                    error.message,
                    true
                );
            } finally {
