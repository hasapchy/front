<template>
    <div class="flex justify-between items-center">
        <PerPageSelector v-if="showPerPageSelector" :per-page="perPage" :per-page-options="perPageOptions"
            :storage-key="storageKey" @per-page-change="$emit('perPageChange', $event)" />
        ...
        <nav class="ml-4">
            <ul class="flex items-center -space-x-px h-8 text-sm">
                ...
                <template v-for="(item, index) in paginationItems">
                    <li v-if="item.type === 'page'" :key="`page-${index}`">
                        <button type="button" :disabled="currentPage === item.value"
                    ...
                    <li v-else-if="item.type === 'ellipsis'" :key="`ellipsis-${index}`">
                ...
                <li>
                    <button type="button" href="#"
                        :class="{ 'opacity-50 cursor-not-allowed': currentPage === lastPage, 'cursor-pointer': currentPage !== lastPage }"
