<template>
  <div
    ...
    class="mt-2"
  >
    <template v-if="orderId || contractId || warehouseReceiptId || selectedSource">
      ...
      <div class="rounded border border-gray-200 bg-white p-3 dark:border-[var(--border-subtle)] dark:bg-[var(--input-bg)]">
        ...
        <div class="mt-1 text-sm text-gray-900 dark:text-[var(--text-primary)]">
          <span class="font-semibold">ID:</span> {{ `#${selectedSource?.id || orderId || contractId || warehouseReceiptId}` }}
        </div>
      </div>
    </template>
  </div>
